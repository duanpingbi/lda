...
var documentsURL = "../download/download.txt";
var stopwordsURL = "/lib/stoplist.txt";
...
d3.select("#showStops").on("click", function () {
  if (displayingStopwords) {
    displayingStopwords = false;
    this.innerText = "显示停用词";
    vocabTable();
  }
  else {
    displayingStopwords = true;
    this.innerText = "隐藏停用词";
    vocabTable();
  }
});
d3.select("#sortVocabByTopic").on("click", function () {
  if (sortVocabByTopic) {
    sortVocabByTopic = false;
    this.innerText = "按主题排序";
    vocabTable();
  }
  else {
    sortVocabByTopic = true;
    this.innerText = "按频率排序";
    vocabTable();
  }
});
...
function ready(error, stops, lines) {
  if (error) { alert("文件上传失败，请重试"); throw error;}
  else {
    // Create the stoplist
    console.log(stops);
